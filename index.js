...
async function addCoordinatesToBoard(boardBuffer) {
    ...
    const canvas = createCanvas(boardSize + margin, boardSize + margin);
    const ctx = canvas.getContext('2d');

    ctx.fillStyle = '#2B2D31';
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    const image = await loadImage(boardBuffer);
    ctx.drawImage(image, margin, 0, boardSize, boardSize);
    ...
    for (let i = 0; i < 8; i++) {
        ctx.fillText((8 - i).toString(), margin / 2, (i * squareSize) + (squareSize / 2));
        ctx.fillText(files[i], margin + (i * squareSize) + (squareSize / 2), boardSize + (margin / 2));
    }
...
client.on('ready', async () => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder()
            .setName('satranç')
            .setDescription('Sunucudan birine satranç oynama isteği gönderir.')
            .addUserOption(o => o.setName('rakip').setDescription('Oynamak istediğiniz kullanıcı').setRequired(true))
    ];
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'satranç') {
            const rakip = options.getUser('rakip');

            if (rakip.bot || rakip.id === interaction.user.id) {
                return interaction.reply({ content: 'Kendinize veya bir bota karşı oynayamazsınız.', flags: MessageFlags.Ephemeral });
            }

            const teklifRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId(`chess_accept_${interaction.user.id}_${rakip.id}`).setLabel('Kabul Et').setStyle(ButtonStyle.Success),
                new ButtonBuilder().setCustomId(`chess_decline_${interaction.user.id}_${rakip.id}`).setLabel('Reddet').setStyle(ButtonStyle.Danger)
            );

            return interaction.reply({
                content: `<@${rakip.id}>, <@${interaction.user.id}> seni bir satranç maçına davet ediyor! Kabul edersen maç başlayacaktır.`,
                components: [teklifRow]
            });
        }
        ...
        if (commandName === 'yardım') {
            const helpEmbed = createEmbed('📑 Komut Listesi', 'Aşağıda botun kullanılabilir komutları listelenmiştir.', 0x5865F2)
                .addFields(
                    { name: '🛠️ Genel Komutlar', value: '`/yardım`, `/öneri`, `/ping`, `/sunucu-bilgi`, `/kullanıcı-bilgi`, `/medya`, `/satranç`' },
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith('chess_accept_')) {
            const parts = interaction.customId.split('_');
            const challengerId = parts[2];
            const rakipId = parts[3];

            if (interaction.user.id !== rakipId) {
                return interaction.reply({ content: 'Bu teklif size yapılmadı.', flags: MessageFlags.Ephemeral });
            }
            ...
            const playRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId(`chess_move_${matchId}`).setLabel('Hamle Yap').setStyle(ButtonStyle.Primary),
                new ButtonBuilder().setCustomId(`chess_resign_${matchId}`).setLabel('Pes Et').setStyle(ButtonStyle.Danger)
            );

            await interaction.update({
                content: `Maç Başladı! Beyazlar: <@${challengerId}> | Siyahlar: <@${rakipId}>\nSıra: <@${challengerId}> (Beyaz)`,
                files: [attachment],
                components: [playRow]
            });
            return;
        ...
        if (interaction.customId.startsWith('chess_decline_')) {
            const parts = interaction.customId.split('_');
            const challengerId = parts[2];
            const rakipId = parts[3];
            ...
            await interaction.update({ content: `<@${rakipId}>, <@${challengerId}> tarafından yapılan satranç teklifini reddetti.`, components: [] });
            return;
        }

        if (interaction.customId.startsWith('chess_move_')) {
            const matchId = interaction.customId.replace('chess_move_', '');
            const gameData = activeChessGames.get(matchId);

            if (!gameData) return interaction.reply({ content: 'Bu maç aktif değil.', flags: MessageFlags.Ephemeral });

            const currentPlayerId = gameData.players[gameData.turn];
            
            if (interaction.user.id !== currentPlayerId) {
                return interaction.reply({ content: 'Sıra sizde değil!', flags: MessageFlags.Ephemeral });
            }
            ...
            moveModal.addComponents(new ActionRowBuilder().addComponents(moveInput));
            await interaction.showModal(moveModal);
            return;
        }

        if (interaction.customId.startsWith('chess_resign_')) {
            const matchId = interaction.customId.replace('chess_resign_', '');
            const gameData = activeChessGames.get(matchId);
            ...
            await interaction.update({
                content: `Maç bitti! <@${interaction.user.id}> pes etti. Kazanan: <@${winnerId}> 🎉`,
                components: [],
                files: []
            });
            return;
    ...
    if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId.startsWith('modal_chess_')) {
            const matchId = interaction.customId.replace('modal_chess_', '');
            const gameData = activeChessGames.get(matchId);

            if (!gameData) {
                return interaction.reply({ content: 'Maç bulunamadı veya sona ermiş.', flags: MessageFlags.Ephemeral });
            }

            const moveText = interaction.fields.getTextInputValue('move_input');
            ...
            await gameData.generator.loadFEN(gameData.chess.fen());
            let buffer = await gameData.generator.generateBuffer();
            buffer = await addCoordinatesToBoard(buffer);
            const attachment = new AttachmentBuilder(buffer, { name: 'board.png' });

            let content = `Beyazlar: <@${gameData.players.w}> | Siyahlar: <@${gameData.players.b}>\n`;

            const isGameOver = (gameData.chess.game_over && gameData.chess.game_over()) || (gameData.chess.isGameOver && gameData.chess.isGameOver());
            const isCheckmate = (gameData.chess.in_checkmate && gameData.chess.in_checkmate()) || (gameData.chess.isCheckmate && gameData.chess.isCheckmate());
            const isDraw = (gameData.chess.in_draw && gameData.chess.in_draw()) || (gameData.chess.isDraw && gameData.chess.isDraw());
            const isStalemate = (gameData.chess.in_stalemate && gameData.chess.in_stalemate()) || (gameData.chess.isStalemate && gameData.chess.isStalemate());
            const isCheck = (gameData.chess.in_check && gameData.chess.in_check()) || (gameData.chess.isCheck && gameData.chess.isCheck());
            ...
            if (isCheckmate) {
                ...
                content += `Sıra: <@${gameData.players[gameData.turn]}>`;
            }

            const playRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder().setCustomId(`chess_move_${matchId}`).setLabel('Hamle Yap').setStyle(ButtonStyle.Primary).setDisabled(isGameOver),
                new ButtonBuilder().setCustomId(`chess_resign_${matchId}`).setLabel('Pes Et').setStyle(ButtonStyle.Danger).setDisabled(isGameOver)
            );
